...
- **Bounty Radar Snapshot**: The interactive browser is refreshed daily at 02:00 UTC
...
`radar/` holds a single-page browser over the same data, served from GitHub Pages at `/radar/`. The markdown tables are the source of truth and are exact; they are less good at answering "what should I work on?", because they cannot be sorted, filtered to your stack, or diffed against last week. The Radar reads `data/all.md` and answers that, with no backend and no build step.

It is refreshed by `.github/workflows/radar-snapshot.yml` at 02:00 UTC — clear of the 00:00 board regeneration and the 09:00 submission triage — via `radar/scripts/archive.js`, which:

1. Reads `data/all.md` and `data/currency_prices.md` straight from the checkout, so it always sees exactly the committed board rather than a half-updated one
2. Writes a dated snapshot into `radar/history/` and recomputes the 7- and 30-day baselines behind the "what changed" view
3. Appends today's sample to the trend series
4. Reads the submission dashboards for the pipeline and payments view
5. Sweeps linked pull requests to flag contested bounties and repository merge throughput
6. Rewrites the data blobs embedded in `radar/index.html` between the `/*NAME_START*/` markers

Details and the full feature list are in [`radar/README.md`](../radar/README.md).

Two properties are worth knowing when reading the page. Each layer carries its own date, because the ↻ button re-reads the board while the sweeps only move when the daily job runs; the timestamp goes amber when they diverge instead of backdating everything to one reassuring value. And a missing contention warning means "not checked yet", never "clear" — the sweeps keep their previous results when they fail rather than publishing an all-clear they did not verify.
